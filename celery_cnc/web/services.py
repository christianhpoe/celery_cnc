...
def db_path() -> Path:
    """Return the configured database path for the CnC store."""
    raw_path = getattr(settings, "CELERY_CNC_DB_PATH", None)
    return Path(raw_path) if raw_path is not None else Path("celery_cnc.db")


def retention_days() -> int:
    """Return the configured retention window in days."""
    return int(getattr(settings, "CELERY_CNC_RETENTION_DAYS", 7))
...
def get_registry() -> WorkerRegistry:
    ...
    for path in getattr(settings, "CELERY_CNC_WORKERS", []):
        if not path:
            continue
        app = _load_app(path)
