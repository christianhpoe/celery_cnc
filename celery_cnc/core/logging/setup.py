...
from celery_cnc.config import CeleryCnCConfig, LoggingConfigFile, get_settings
...
def configure_process_logging(
    config: CeleryCnCConfig | LoggingConfigFile | None = None,
    *,
    component: str | None = None,
) -> None:
    """Ensure log handlers are configured for the current process."""
    resolved = config or get_settings()
    log_config = resolved.logging if isinstance(resolved, CeleryCnCConfig) else resolved
